...
export async function createFrontPage() {
  return Word.run(async (context) => {
    ...
    const paragraph = context.document.body.insertHtml("<br> <div style='text-align:center'>FRONT PAGE</div> <br> FRONT <br> FROAAANT" + String(Date()), Word.InsertLocation.start);
...
export async function createHeader() {
  return Word.run(async (context) => {
    ...
    const paragraph = header.insertParagraph("Name: _______________________     Datum: _______________________", Word.InsertLocation.end);
...
export async function createFooter() {
  return Word.run(async (context) => {
    ...
    footer.insertInlinePictureFromBase64(getQrCodeBase64(),Word.InsertLocation.start)
    paragraph.font.bold = true;
