...
export default function DocumentTitlePage() {
  ...
  return(
    <div className="centerTopPadding">
      ...
      <PrimaryButton text="Titelseite erstellen" id="frontPageCreate" onClick={() => _createFrontPage(examDate, courseName, examName)} />
    </div>
  );
}

function _createFrontPage(_examDate : Date, _courseName, _examName) {
  const headerHtml = "<br> <div style='text-align:center'> <div style='font-size: x-large'>" + _examName + " </div> <br> <br> " + _courseName + " <br> " + String(_examDate.toLocaleDateString("de-De") + "</div><br> <br> <br>");
  
  Word.run(async (context) => {
    var tableParagraph = context.document.body.paragraphs.getFirst();
    ...
    try{
      ...
      var table = tableParagraph.insertTable(1, 2, Word.InsertLocation.before, null);
